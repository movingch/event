...
function doPost(e) {
  ...
  try {
    ...
    writeDonationsSheet(ss, data.donations, now);
...
function extractData_(body) {
  ...
  const donations = firstNonEmptyArray_(normalizeRecords(body.donations), normalizeRecords(body.rows && body.rows.donations), normalizeRecords(body.data && body.data.donations), normalizeRecords(body.sheets && body.sheets.donations));
...
function writeDonationsSheet(ss, donations, now) {
  const sheet = resetSheet_(ss, '후원자현황');
  const headers = ['No', '신청일시', '후원자명', '입금자명', '연락처', '후원금액', '문자상태', '문자발송일', '문자요청ID', '오류메시지', '저장시각'];
  const rows = (donations || []).map(function (d, index) {
    return [
      index + 1,
      valueOf(d, ['createdAt', '신청일시']),
      valueOf(d, ['donorName', '후원자명']),
      valueOf(d, ['depositorName', '입금자명']),
      valueOf(d, ['phone', '연락처']),
      valueOf(d, ['amount', '후원금액']) || 10000,
      valueOf(d, ['smsStatus', '문자상태']),
      valueOf(d, ['smsSentAt', '문자발송일']),
      valueOf(d, ['smsRequestId', '문자요청ID']),
      valueOf(d, ['smsError', '오류메시지']),
      formatDateTime(now)
    ];
  });
  const total = rows.reduce(function (sum, row) { return sum + Number(row[5] || 0); }, 0);
  writeDesignedTable_(sheet, '후원자 신청 현황', '후원 신청 ' + rows.length + '건 · 후원 예정액 ' + total.toLocaleString('ko-KR') + '원 · 기준시각: ' + formatDateTime(now), headers, rows, 4);
  paintStatusColumn_(sheet, 5, 7, rows.length);
  try { sheet.getRange(5, 6, Math.max(rows.length, 1), 1).setNumberFormat('#,##0'); } catch (err) {}
}
...
function applySpreadsheetFinishing_(ss) {
  const order = ['신청자현황', '신청자현황_출력용', '통계', '상영관영화', '후원자현황', '만족도_설정', '만족도_문항', '만족도_응답', '만족도_통계', '만족도_문자발송기록', '저장기록', '저장오류'];
