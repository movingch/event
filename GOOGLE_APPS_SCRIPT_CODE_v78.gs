...
function doGet(e) {
  ...
  return jsonOutput({ ok: true, message: '머내마을영화제 구글시트 연동 웹앱 v77 만족도조사 시트 디자인 버전이 작동 중입니다.' });
}
...
function writeApplicantsSheet(ss, applicants, now) {
  ...
  const canceled = rows.filter(function (row) { return String(row[3]).indexOf('미참석') >= 0 || String(row[3]).indexOf('취소') >= 0; }).length;
  const title = '제9회 머내마을영화제 신청자현황';
  const summary = '기준시각: ' + formatDateTime(now) + '    총 신청자: ' + rows.length + '명    참석: ' + attended + '명    미참석/취소: ' + canceled + '명';
...
function writePrintableApplicantsSheet(ss, applicants, now) {
  ...
  setDropdown_(sheet, 4, 10, ['참석', '미참석', '확인필요']);
...
function paintStatusColumn_(sheet, startRow, column, rowCount) {
  ...
  const colors = values.map(function (row) {
    ...
    if (v.indexOf('미발송') >= 0 || v.indexOf('예정') >= 0 || v.indexOf('확인') >= 0) return [BRAND.amberBg];
    return [BRAND.grayBg];
  });
