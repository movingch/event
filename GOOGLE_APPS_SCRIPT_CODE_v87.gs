...
// v87: 자동 저장 시 사용자가 직접 조정한 구글시트 열 너비를 보존합니다.
// 열 너비는 최초 시트 생성 후 구글시트에서 직접 조정해도 다음 저장 때 되돌아가지 않습니다.
...
function doPost(e) {
  ...
  try {
    ...
    return jsonOutput({
      ...
      version: 'v87-preserve-column-widths',
...
function doGet(e) {
  ...
  return jsonOutput({ ok: true, message: '머내마을영화제 구글시트 연동 웹앱 v87 열 너비 보존 버전이 작동 중입니다.' });
}
...
function styleSurveyResponseAppend_(sheet) {
  try {
    ...
    if (lastRow >= 5 && lastCol >= 17) {
      ...
      // v87: 사용자가 조정한 열 너비를 보존하기 위해 자동 열폭 조정을 하지 않습니다.
      // v87: 수동 열 너비 보존
      // v87: 수동 열 너비 보존
    }
  } catch (err) {}
