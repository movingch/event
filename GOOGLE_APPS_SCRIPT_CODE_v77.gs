...
function writeApplicantsSheet(ss, applicants, now) {
  ...
  setDropdown_(sheet, 4, 17, ['미발송', '발송완료', '실패']);
...
function writeSurveyResponsesSheet(ss, responses, now) {
  ...
  const headers = ['No', '응답시각', '예약번호', '설문토큰', '이름', '연락처', '영화명', '상영일시', '장소', '좌석/인원', '전체만족도', '상영환경', '진행안내', '재참여의향', '좋았던점', '개선의견', '응답상태'];
