...
const OPENING_HEADLINE = "박정민 배우, 머내마을 개막식에 오다";
const OPENING_HEADLINE_LINES = ["박정민 배우,", "머내마을", "개막식에 오다"];
const EARLYBIRD_MESSAGE = "박정민배우를 가장 가까이서 만날 수 있는 후원자님들을 위한 선예약";
...
const seedOpeningScreening = {
  ...
  staffPin: "0909",
  status: "신청 가능",
  notes: "박정민 배우, 머내마을 개막식에 오다. 9월 9일 저녁 7시 동천농협강당에서 열리는 개막식 상영입니다. 후원자 선예약은 지정좌석, 일반석은 자유석으로 운영합니다.",
  isOpening: true,
  guest: "박정민 배우",
...
function normalizeScreening(screening) {
  ...
  if (isOpening) {
    ...
    if (!screening?.notes || screening.notes === "박정민 배우 참석 예정. 후원자 선예약은 지정좌석, 일반 관객은 자유석으로 운영합니다.") merged.notes = seedOpeningScreening.notes;
...
function openingPhaseInfo(screening = getOpeningScreening()) {
  ...
  if (screening.status === "준비중" || (earlyStart && now < earlyStart)) {
    return { phase: "before", label: "후원자 선예약 준비중", className: "warn", allowBooking: false, ticketType: "", message: `${formatDateTime(screening.earlyBirdStart)}부터 ${EARLYBIRD_MESSAGE}이 열립니다.` };
  }
  if (!earlyEnd || now <= earlyEnd) {
    return { phase: "earlybird", label: "후원자 선예약 진행중", className: "ok", allowBooking: true, ticketType: "얼리버드", seatType: "지정좌석", message: `${EARLYBIRD_MESSAGE}입니다. 확정 시 지정좌석이 자동 배정됩니다.` };
  }
  ...
  return { phase: "general", label: "일반석 오픈", className: "blue", allowBooking: true, ticketType: "일반", seatType: "자유석", message: "후원자 선예약 기간이 끝나 일반석 자유석 신청이 가능합니다." };
}
  ...
  return `
    <section class="opening-home-hero">
      <div class="opening-home-copy">
        ...
        <p>${esc(EARLYBIRD_MESSAGE)}입니다. 개막식 당일까지 메인 화면에서 개막식 예매를 가장 먼저 안내합니다. 후원자 선예약은 지정좌석으로, 일반석 오픈 이후 신청자는 자유석으로 운영됩니다.</p>
        ...
        <div class="cta-row">
          ...
          <a class="btn btn-ghost" href="#opening-video">소개영상 보기</a>
          <button class="btn btn-primary" type="button" data-action="donate">후원하기</button>
        </div>
  ...
  return `
    <section class="opening-inline-card">
      <div>
        ...
        <h2>박정민 배우, 머내마을 개막식에 오다</h2>
        <p>개막식 영화 &lt;얼굴&gt; · 9월 9일 저녁 7시 동천농협강당 · 박정민 배우 참석. 후원자 선예약은 지정좌석으로, 일반석 오픈 후 신청자는 자유석으로 운영됩니다. 지정좌석 잔여 ${stats.designatedRemaining}석 · 전체 잔여 ${stats.remainingTotal}석</p>
      </div>
      <div class="cta-row">
  ...
  return `
    <section class="opening-hero">
      <div class="opening-hero-main">
        ...
        <h1 class="opening-title-stack opening-title-detail">${openingHeadlineMarkup()}<span class="opening-title-sub">개막식 영화 &lt;얼굴&gt; · 후원자 선예약</span></h1>
...
function screeningCard(screening) {
  const confirmed = confirmedSeats(screening.id);
  const wait = waitlistSeats(screening.id);
  const rate = Math.min(occupancyRate(screening), 120);
  ...
  const barClass = info.className === "danger" ? "danger" : info.className === "warn" ? "warn" : "";
  return `
    <article class="screening-card ${isOpening ? "opening-card" : ""}" data-screening-card="${esc(screening.id)}">
      <div class="screening-top">
        <div class="badges">
          ...
          ${wait > 0 ? `<span class="badge danger">대기 ${wait}명</span>` : ""}
        </div>
      ...
      <div class="screening-body">
        <div class="progress-wrap" aria-label="정원 현황">
          <div class="progress-label"><span>확정 ${confirmed}명</span><span>정원 ${Number(screening.capacity || 0)}명 · ${occupancyRate(screening)}%</span></div>
          <div class="progress"><span class="${barClass}" style="width:${Math.min(rate, 100)}%"></span></div>
        </div>
        ${isOpening ? `
          <div class="opening-card-mini">
            <span>박정민 배우 참석</span>
            <strong>얼리버드 ${openStats.earlybirdSeats}명 · 일반 ${openStats.generalSeats}명</strong>
            <small>후원자 지정좌석 / 일반석 자유석</small>
          </div>
        ` : ""}
        <div class="info-list">
          <div class="info-row"><span>GV 담당</span><strong>${esc(screening.gvHost || "없음")}</strong></div>
          <div class="info-row"><span>모더레이터</span><strong>${esc(screening.moderator || "미정")}</strong></div>
          <div class="info-row"><span>담당 스태프</span><strong>${esc(screening.staff || "미정")}</strong></div>
        </div>
        ${screening.notes ? `<p class="help">${esc(screening.notes)}</p>` : ""}
        <div class="screening-card-actions">
          <button class="btn btn-dark" type="button" data-action="book" data-id="${esc(screening.id)}" ${isOpening && !phase.allowBooking ? "disabled" : ""}>${isOpening ? (phase.allowBooking ? "개막작 신청하기" : phase.label) : "이 영화 신청하기"}</button>
          <a class="btn btn-outline" href="#/staff/${esc(screening.id)}">신청자 확인</a>
        </div>
      </div>
    </article>
  `;
...
function staffScreenings() {
  ...
  if (!session) return [];
  return sortedScreenings().filter((screening) => session.screeningIds.includes(screening.id));
}
  ...
  return `
    <section class="section-title staff-portal-title">
      <div>
        ...
        <h1>${esc(session.staffName)} 스태프 신청자 관리</h1>
        <p>담당 상영관의 신청자 확인, 확정·대기·취소 처리, 문자 재발송, 현장 참석 확인을 할 수 있습니다.</p>
      </div>
      <div class="cta-row">
    ...
    <section class="stats-grid staff-stats">
      <div class="stat-card"><span>담당 회차</span><strong>${screenings.length}</strong><small>${screenings.map((s) => esc(s.venue)).join(" · ")}</small></div>
      <div class="stat-card"><span>신청 건수</span><strong>${reservations.length}</strong><small>취소 포함</small></div>
      <div class="stat-card"><span>신청 인원</span><strong>${totalSeats}</strong><small>취소 제외</small></div>
      <div class="stat-card"><span>참석 확인</span><strong>${attendedSeats}</strong><small>실제 참석 인원</small></div>
    </section>
    ${screenings.map((screening) => `
      <section class="card staff-screening-card">
        <div class="section-title">
          <div>
            <div class="badges"><span class="badge blue">${esc(screening.venue)}</span><span class="badge ${statusInfo(screening).className}">${esc(statusInfo(screening).text)}</span></div>
            <h2>${esc(screening.title)}</h2>
...
function renderStaffLogin(preselectedId = "") {
  const options = sortedScreenings().map((screening) => `<option value="${esc(screening.id)}" ${screening.id === preselectedId ? "selected" : ""}>${esc(screening.title)} · ${esc(screening.venue)} · ${esc(screening.staff || "담당 미정")}</option>`).join("");
  return `
    <section class="admin-login staff-login">
      <div class="login-card">
        <div class="eyebrow">상영관 담당자 전용</div>
        <h1>신청자 확인</h1>
        <p>담당 회차를 선택하고 관리자가 부여한 스태프 비밀번호를 입력하세요. 같은 담당자·같은 비밀번호로 지정된 회차는 함께 관리할 수 있습니다.</p>
...
function submitStaffLogin(form) {
  ...
  const matching = state.screenings.filter((item) => item.staff && item.staff === screening.staff && item.staffPin && item.staffPin === screening.staffPin);
  sessionStorage.setItem(STAFF_SESSION_KEY, JSON.stringify({
    staffName: screening.staff || "담당 스태프",
    screeningIds: matching.map((item) => item.id),
          ...
          ${screenings.map((screening) => {
            ...
            return `
              <tr>
                ...
                <td><strong>${esc(screening.title)}</strong><br><span class="help">${esc(screening.venue)}</span>${isOpening ? `<br><span class="help">얼리버드 ${openStats.earlybirdSeats}명 · 일반 ${openStats.generalSeats}명 · 지정잔여 ${openStats.designatedRemaining}석</span>` : ""}</td>
  ...
  return `
    <section class="card opening-admin-card">
      <div class="section-title">
        <div>
          ...
          <p>개막식 “얼굴”, 박정민 배우 참석, 메인 화면 노출 종료일, 포스터·소개영상, 후원자 선예약 기간, 일반석 오픈 기간, 지정좌석 수를 관리합니다.</p>
        </div>
      ...
      <form id="openingForm">
        <div class="form-grid">
          ...
          <div>
            <label class="label" for="earlyBirdStart">후원자 선예약 시작</label>
          ...
          <div>
            <label class="label" for="earlyBirdEnd">후원자 선예약 종료</label>
    ...
    <section class="card">
      <div class="section-title">
        <div>
          <h2>개막작 신청자 현황</h2>
          <p>후원자 선예약은 지정좌석, 일반석은 자유석으로 표시됩니다.</p>
        </div>
...
function adminReservations() {
  const options = sortedScreenings().map((s) => `<option value="${esc(s.id)}">${esc(s.title)} · ${esc(s.venue)}</option>`).join("");
  return `
    <section class="card">
      <div class="section-title">
        <div>
          <h2>신청자 명단</h2>
          <p>누가 어느 영화와 상영관에 신청했는지 검색하고, 현장에서 참석 확인 버튼으로 실제 참석 인원을 기록할 수 있습니다. ‘확인문구’ 버튼으로 카톡·문자·메일에 붙여넣을 예매 확인 문구를 복사할 수 있습니다.</p>
        </div>
        <div class="cta-row">
          <button class="btn btn-dark" type="button" data-action="print">인쇄용 명단</button>
          <button class="btn btn-outline" type="button" data-action="export-reservations">신청자 CSV</button>
      ...
      <div class="print-only print-heading">
        <h2>오프라인 참석 체크 명단</h2>
        <p>현장에서는 ‘참석 체크’ 칸에 표시하고, 운영 종료 후 대시보드의 참석 확인 버튼으로 실제 참석 인원을 입력하세요.</p>
      </div>
  ...
  return `
    <div class="table-wrap reservation-table-wrap">
      <table class="reservation-table">
        <thead>
          <tr>
            <th>상태</th><th>예약번호</th><th>참석</th><th>신청자</th><th>영화 / 상영관</th><th>티켓/좌석</th><th>인원</th><th>문자</th><th>신청일</th><th>메모</th><th class="screen-only">관리</th>
          </tr>
        </thead>
        <tbody>
            ...
            return `
              <tr class="${attended ? "attended-row" : ""}">
                ...
                <td class="attendance-cell">
                  ...
                  <div class="print-only print-check"><span class="check-box">${attended ? "✓" : ""}</span><span>참석 체크</span></div>
                </td>
                ...
                <td>${screening ? `<strong>${esc(screening.title)}</strong><br><span class="help">${esc(screening.venue)} · ${esc(formatDateTime(screening.startTime))}</span>` : "삭제된 회차"}</td>
                ...
                <td class="sms-status-cell"><span class="badge ${smsStatusClass(reservation)}" data-sms-status-for="${esc(reservation.id)}">${esc(smsStatusText(reservation))}</span></td>
                ...
                <td class="screen-only">
                  <div class="row-actions">
                    ...
                    <button class="btn btn-outline btn-small" type="button" data-action="copy-confirmation" data-id="${esc(reservation.id)}">확인문구</button>
                    <button class="btn btn-outline btn-small" type="button" data-action="send-sms" data-id="${esc(reservation.id)}">문자발송</button>
                    ${options.staffMode ? "" : `<button class="btn btn-danger btn-small" type="button" data-action="delete-reservation" data-id="${esc(reservation.id)}">삭제</button>`}
                  </div>
                </td>
              </tr>
            `;
...
function adminStats() {
  ...
  return `
    ...
    <section class="card">
      ...
      <div class="opening-summary-grid">
        <div class="metric-card"><div class="metric-label">후원자 선예약 확정</div><div class="metric-value">${openingStats(getOpeningScreening()).earlybirdSeats}</div><div class="metric-note">지정좌석 잔여 ${openingStats(getOpeningScreening()).designatedRemaining}석</div></div>
...
function openingBookingIntro(screening, phase, stats) {
  if (phase.phase === "earlybird") {
    return `
      <div class="opening-booking-intro earlybird">
        <div class="badges"><span class="badge ok">얼리버드</span><span class="badge warn">후원자 선예약</span><span class="badge blue">지정좌석</span></div>
  ...
  return `
    <div class="opening-booking-intro general">
      ...
      <h3>일반석 신청이 열렸습니다.</h3>
      <p>후원자 선예약 기간이 종료되어 자유석으로 접수됩니다. 현장에서 예약번호와 신청자 이름을 알려주세요.</p>
    </div>
  `;
}
