...
const LEGACY_WAIT_STATUS = ["대", "기"].join("");
...
function normalizeReservation(reservation = {}) {
  ...
  const normalizedStatus = String(base.status || "") === "취소" ? "취소" : "확정";
  return {
    ...base,
    ...
    smsStatus: String(base.smsStatus || "").includes(LEGACY_WAIT_STATUS) ? "미발송" : (base.smsStatus || "미발송"),
    ...
    smsHistory: Array.isArray(base.smsHistory) ? base.smsHistory.map((entry, index) => ({
      ...
      status: String(entry?.status || "").includes(LEGACY_WAIT_STATUS) ? "미발송" : String(entry?.status || "발송완료"),
...
function screeningDateParts(value) {
  if (!value) return { date: "날짜 미정", day: "날짜", weekday: "미정", corner: "날짜 미정" };
  const date = new Date(value);
  if (Number.isNaN(date.getTime())) return { date: String(value), day: String(value), weekday: "", corner: String(value) };
...
function normalizedBookingGroup(value = "") {
  return String(value || "").trim().replace(/\s+/g, " ").toLocaleLowerCase("ko-KR");
}
...
function screeningConflictGroupIds(target, screenings = state.screenings) {
  ...
  const groupIds = new Set([target.id]);
  const queue = [target];
  while (queue.length) {
    const current = queue.shift();
    screenings.forEach((candidate) => {
      if (!candidate || groupIds.has(candidate.id)) return;
      if (!screeningTimesConflict(current, candidate)) return;
...
function groupScreeningsByBookingConflict(screenings = []) {
  const remaining = [...screenings];
  const groups = [];
  while (remaining.length) {
    ...
    for (let index = 0; index < group.length; index += 1) {
      ...
      for (let candidateIndex = remaining.length - 1; candidateIndex >= 0; candidateIndex -= 1) {
        if (!screeningTimesConflict(current, remaining[candidateIndex])) continue;
        group.push(remaining.splice(candidateIndex, 1)[0]);
      }
    }
    group.sort(compareScreeningStartTime);
    groups.push(group);
  }
  return groups;
}
...
function groupScreeningsForSchedule(screenings = []) {
  ...
  screenings.forEach((screening) => {
    ...
    if (!forcedGroups.has(key)) forcedGroups.set(key, { key, label: String(screening.bookingGroup || "").trim(), screenings: [] });
    forcedGroups.get(key).screenings.push(screening);
  });
  const groups = groupScreeningsByBookingConflict(automaticScreenings).map((group) => ({
    forced: false,
    label: "",
    screenings: group
  }));
  forcedGroups.forEach((group) => groups.push({
    forced: true,
    label: group.label,
    screenings: group.screenings.sort(compareScreeningStartTime)
  }));
...
function scheduleLayoutBlockId(group = {}) {
  const ids = [...(group.screenings || [])].map((screening) => String(screening.id || "")).filter(Boolean).sort();
  if (group.forced && group.label) return `group:${normalizedBookingGroup(group.label)}`;
  return `${group.released ? "released" : (ids.length > 1 ? "conflict" : "screening")}:${ids.join("|")}`;
}

function scheduleLayoutBlockDimensions(block = {}) {
  const visualCount = block.repeatSeries ? 1 : Math.max(1, block.screenings.length);
  if (block.orientation === "vertical") return { columns: 1, rows: visualCount };
  const columns = Math.min(3, visualCount);
  return { columns, rows: Math.max(1, Math.ceil(visualCount / columns)) };
}

function scheduleLayoutBlocks(screenings = [], options = {}) {
  const columns = 3;
  const groups = groupScreeningsForSchedule(screenings).map((group) => {
    const repeatSeries = continuousRepeatSeries(group.screenings);
    group.screenings = [...group.screenings].sort(compareScreeningStartTime);
    const anchor = group.screenings[0] || {};
    const block = {
      ...group,
      repeatSeries,
      id: scheduleLayoutBlockId(group),
      orientation: anchor.scheduleGroupOrientation === "vertical" ? "vertical" : "horizontal",
      requestedRow: screeningScheduleGridNumber(anchor.scheduleRow),
      requestedColumn: screeningScheduleGridNumber(anchor.scheduleColumn, 3)
    };
    return { ...block, ...scheduleLayoutBlockDimensions(block) };
  });
  const occupied = new Set();
  const fits = (row, column, width, height) => {
    if (row < 1 || column < 1 || column + width - 1 > columns) return false;
  ...
  const occupy = (block, row, column) => {
    block.row = row;
    block.column = column;
    for (let r = row; r < row + block.rows; r += 1) {
      for (let c = column; c < column + block.columns; c += 1) occupied.add(`${r}:${c}`);
    }
  };
  const firstOpen = (block) => {
    for (let row = 1; row < 200; row += 1) {
      for (let column = 1; column <= columns; column += 1) {
        if (fits(row, column, block.columns, block.rows)) return { row, column };
      }
    }
    return { row: 1, column: 1 };
  };
  blocksWithManualPositionsFirst(blocksSortedForLayout(groups)).forEach((block) => {
    const manual = block.requestedRow && block.requestedColumn
      && fits(block.requestedRow, block.requestedColumn, block.columns, block.rows)
      ? { row: block.requestedRow, column: block.requestedColumn }
      : null;
    const position = manual || firstOpen(block);
    occupy(block, position.row, position.column);
  });
  const maxRow = groups.reduce((max, block) => Math.max(max, block.row + block.rows - 1), 1);
  const rowCount = Math.max(2, maxRow + Math.max(0, Number(options.extraRows || 0)));
  return { columns, rowCount, blocks: groups, occupied };
}

function screeningScheduleGridNumber(value, maximum = Number.MAX_SAFE_INTEGER) {
  if (value === "" || value == null || !Number.isFinite(Number(value))) return null;
  return Math.max(1, Math.min(maximum, Math.round(Number(value))));
}

function blocksSortedForLayout(blocks = []) {
  return [...blocks].sort((first, second) => {
    const firstOrder = Math.min(...first.screenings.map((screening) => screeningScheduleOrder(screening)));
    const secondOrder = Math.min(...second.screenings.map((screening) => screeningScheduleOrder(screening)));
    return (firstOrder - secondOrder)
      || String(first.screenings[0]?.startTime || "").localeCompare(String(second.screenings[0]?.startTime || ""));
  });
}

function compareScreeningStartTime(first, second) {
...
function blocksWithManualPositionsFirst(blocks = []) {
  return [...blocks].sort((first, second) => {
    const firstManual = first.requestedRow && first.requestedColumn ? 0 : 1;
    const secondManual = second.requestedRow && second.requestedColumn ? 0 : 1;
    return firstManual - secondManual || blocks.indexOf(first) - blocks.indexOf(second);
  });
}

function scheduleLayoutBlockChoiceHtml(block = {}) {
  if (block.repeatSeries) return `<strong>${block.repeatSeries.slotCount}개 회차 중 1회만 예약 가능</strong><span>${esc(continuousSeriesTimeLabel(block.repeatSeries))} · ${esc(continuousSeriesIntervalLabel(block.repeatSeries))}</span>`;
  if (block.forced) return `<strong>${block.screenings.length}개 회차 중 1회만 예약 가능</strong><span>${esc(block.label)}</span>`;
  if (block.screenings.length > 1) return `<strong>${block.screenings.length}편 중 1편 선택 가능</strong><span>동시간대·앞뒤 1시간 이내 중복신청 제한</span>`;
  return `<strong>선택 제한 없음</strong><span>단독 상영</span>`;
}

function scheduleLayoutBlockHtml(block, options = {}) {
  const adminArrange = options.adminArrange === true;
  const dayKey = options.dayKey || scheduleDayKey(block.screenings[0]);
  const visualCount = block.repeatSeries ? 1 : block.screenings.length;
  const innerColumns = block.orientation === "vertical" ? 1 : Math.min(3, Math.max(1, visualCount));
  const innerRows = block.orientation === "vertical" ? Math.max(1, visualCount) : Math.max(1, Math.ceil(visualCount / innerColumns));
  const choicePlaceholder = block.forced || block.screenings.length > 1 ? "" : "is-placeholder";
  const groupClass = block.forced ? "is-forced-group" : (block.screenings.length > 1 ? "is-conflict-group" : "");
  return `
    <section class="classic-schedule-time-group schedule-layout-block schedule-layout-${block.orientation} ${groupClass} ${adminArrange ? "schedule-layout-edit-block" : ""}"
      style="grid-column:${block.column} / span ${block.columns};grid-row:${block.row} / span ${block.rows};--booking-group-columns:${innerColumns};--booking-group-rows:${innerRows}"
      data-schedule-block-id="${esc(block.id)}" data-schedule-day="${esc(dayKey)}" data-schedule-block-row="${block.row}" data-schedule-block-column="${block.column}" ${adminArrange ? `draggable="true" tabindex="0"` : ""}>
      ${adminArrange ? `<div class="schedule-layout-block-toolbar">
        <span class="schedule-layout-block-drag">⋮⋮ <b>${visualCount > 1 ? "그룹 전체 이동" : "카드 이동"}</b></span>
        ${visualCount > 1 ? `<div class="schedule-layout-direction-buttons" aria-label="배치 방향 선택">
          <button type="button" data-action="set-schedule-block-orientation" data-block-id="${esc(block.id)}" data-day="${esc(dayKey)}" data-orientation="horizontal" ${block.orientation === "horizontal" ? "disabled" : ""}>가로</button>
          <button type="button" data-action="set-schedule-block-orientation" data-block-id="${esc(block.id)}" data-day="${esc(dayKey)}" data-orientation="vertical" ${block.orientation === "vertical" ? "disabled" : ""}>세로</button>
        </div>` : ""}
      </div>` : ""}
      <div class="classic-schedule-group-choice ${choicePlaceholder}" ${choicePlaceholder ? `aria-hidden="true"` : ""}>${scheduleLayoutBlockChoiceHtml(block)}</div>
      <div class="screening-grid classic-schedule-grid">
        ${block.repeatSeries
          ? screeningSeriesCard(block.repeatSeries, { schedule: true, adminArrange, blockChild: adminArrange })
          : block.screenings.map((screening) => screeningCard(screening, { schedule: true, adminArrange, blockChild: adminArrange, groupMember: block.screenings.length > 1 || block.forced })).join("")}
      </div>
    </section>`;
}
...
function continuousSeriesStatus(series) {
  ...
  if (bookable.length > 0) return { className: "danger", text: "신청마감", full: true };
...
function bookingRuleError(screening, name, phone) {
  ...
  const conflict = existing.find((reservation) => {
    ...
    return reservedScreening && (conflictGroupIds.has(reservedScreening.id) || forcedGroupIds.has(reservedScreening.id));
  });
  if (conflict) {
    const reservedScreening = state.screenings.find((item) => item.id === conflict.screeningId);
    if (screeningsShareForcedBookingGroup(screening, reservedScreening)) {
      return `“${screening.bookingGroup}” 그룹에서는 한 번만 예약할 수 있습니다. 이미 신청한 회차: ${formatDateTime(reservedScreening?.startTime || "")} ${cleanMovieTitle(reservedScreening?.title || "상영작")}`;
    }
    return `같은 예약 제한 그룹에서는 한 편만 신청할 수 있습니다. 이미 신청한 영화: ${cleanMovieTitle(reservedScreening?.title || "상영작")}`;
  }
  return "";
}
...
function bookingStatusFor(screening, seats = 1) {
  ...
  if (!capacity) return "마감";
  return confirmedSeats(screening.id) + Math.max(1, Number(seats || 1)) <= capacity ? "확정" : "마감";
}
...
function statusInfo(screening) {
  ...
  if (remaining <= 0) return { text: "신청마감", className: "danger" };
...
function getTotals() {
  const activeReservations = state.reservations.filter((reservation) => !isCanceledReservation(reservation));
  const confirmed = activeReservations;
  const unattended = activeReservations.filter(isUnattendedReservation);
  ...
  const totalUnattendedSeats = unattended.reduce((sum, r) => sum + Number(r.seats || 0), 0);
  ...
  const overCapacityCount = state.screenings.filter((s) => confirmedSeats(s.id) > Number(s.capacity || 0)).length;
  return {
    activeReservations,
    confirmed,
    unattended,
...
function adminPageHref(tab = "overview", master = false) {
  ...
  if (localHost) return `/?v=141&fresh=1&${master ? "masterPreview" : "adminPreview"}=1#/${tab}`;
  return `/${master ? "adminor" : "admin"}?v=141&fresh=1#/${tab}`;
}
...
function appHeader() {
  ...
  return `
    <header class="header">
      <a class="logo" href="/?v=141&fresh=1" data-action="go-home" aria-label="메인화면으로 이동">
        <div class="logo-mark"><img src="assets/munae-horse-logo.png" alt="머내마을영화제 말 캐릭터 로고"></div>
        <div>
          <div class="logo-title">제9회 머내마을영화제 예약시스템</div>
    ...
    <section class="hero">
      <div class="hero-grid">
        <div>
          ...
          <h1 class="hero-main-title"><span>마을 주민이 직접 만드는</span><span>영화제에 함께해주세요.</span></h1>
          <div class="hero-donation-copy">${DONATION_MESSAGE.split(/\n\n+/).map((paragraph) => `<p>${esc(paragraph)}</p>`).join("")}</div>
...
function renderReservationManagePage() {
  ...
  if (!reservationManageSession && !reservationManageChallenge) {
    content = `<form class="reservation-manage-form" id="reservationLookupForm">
      <label class="field full"><span>예약할 때 입력한 휴대폰 번호</span><input class="input reservation-manage-phone" type="tel" name="phone" inputmode="tel" autocomplete="tel" placeholder="휴대폰 번호 입력" required /></label>
...
function renderDonationPage() {
  return `
    <section class="donation-page-hero">
      ...
      <h1>머내마을영화제 후원하기</h1>
      <div class="donation-story">${DONATION_MESSAGE.split(/\n\n+/).map((paragraph) => `<p>${esc(paragraph)}</p>`).join("")}</div>
    </section>
    <section class="card donation-form-card">
      ...
      <form id="donationForm" class="form-grid donation-form">
        ...
        <div class="form-field full-span">
          ...
          <input class="input" id="donorPhone" name="phone" required inputmode="tel" autocomplete="tel" placeholder="휴대폰 번호 입력" />
        </div>
...
function renderScreeningSchedule() {
  ...
  dayGroups.forEach((group) => {
    group.layout = scheduleLayoutBlocks(group.screenings);
  });
  return `
    <section class="screening-schedule-section" aria-labelledby="screeningScheduleTitle">
      ...
      <div class="classic-schedule-day-groups">
        ${dayGroups.map((group, index) => `
          <section class="classic-schedule-day-group schedule-day-color-${(index % 5) + 1}" aria-labelledby="scheduleDay${index}">
            ...
            <div class="classic-schedule-time-groups schedule-layout-grid" style="--schedule-layout-rows:${group.layout.rowCount}">
              ${group.layout.blocks.map((block) => scheduleLayoutBlockHtml(block, { dayKey: group.key })).join("")}
            </div>
          </section>`).join("")}
      </div>
    </section>`;
  ...
  return `
    <section class="full-schedule-page">
      ...
      <div class="timeline-legend" aria-label="시간표 안내">
        ...
        <span><i class="legend-dot is-full"></i>신청 마감</span>
                ...
                ${displayItems.map((item) => {
                  ...
                  return `<button class="festival-text-movie ${full ? "is-full" : ""}" type="button" data-action="book" data-id="${esc(screening.id)}" aria-label="${esc(cleanMovieTitle(screening.title))} ${esc(formatTimePart(screening.startTime))} 관람 신청">
                    <span class="festival-text-movie-topline">
                      ...
                      <em class="${esc(info.className)}">${full ? "신청마감" : "신청가능"}</em>
                    </span>
                    <strong>${esc(cleanMovieTitle(screening.title))}</strong>
                    <span class="festival-text-venue">${esc(screening.venue)}</span>
                    <span class="festival-text-meta">
...
function renderApply() {
  ...
  return `
    <section class="section-title">
      <div>
        <h1>영화 신청하기</h1>
        <p>보고 싶은 영화와 상영관을 선택한 뒤 참석 인원과 연락처를 남겨주세요. 정원이 찬 회차는 신청이 마감됩니다.</p>
      </div>
    </section>
    ...
    <section class="apply-participation-notice" aria-labelledby="applyParticipationNoticeTitle">
      ...
      <p>무분별한 <strong>영화신청으로 인한</strong> 노쇼방지를 위해 <strong>동일한</strong> 시간대 중복 예약을 방지합니다.</p>
    </section>

    ${renderScreeningSchedule()}

    <section class="all-screenings-section" aria-labelledby="allScreeningsTitle">
      <div class="section-title compact-title">
        <div>
          <h2 id="allScreeningsTitle">전체상영</h2>
          <p>날짜·요일·영화제목·상영관을 목록으로 확인하고 원하는 행을 선택해 신청할 수 있습니다.</p>
        </div>
      </div>
      <section class="filters all-screening-filters" aria-label="전체상영 필터">
        ...
        <select class="select" id="seatFilter">
          ...
          <option value="full">신청 마감</option>
        </select>
      </section>
      <section id="screeningList" class="all-screening-list" aria-live="polite"></section>
    </section>
  `;
}

function screeningCard(screening, options = {}) {
  ...
  const adminArrange = options.adminArrange === true;
  const blockChild = options.blockChild === true;
  const full = remainingSeats(screening) <= 0;
  const bookingDisabled = layoutPreview || adminArrange || full || (exposeOpening && phase && !phase.allowBooking);
  const bookingLabel = layoutPreview ? "배치 확인용" : (exposeOpening && phase ? (phase.allowBooking ? (full ? "신청마감" : "관람신청") : phase.label) : (full ? "신청마감" : "관람신청"));
  ...
  return `
    <article class="screening-card compact-screening-card screening-category-color-${categoryColor} ${options.schedule ? "classic-schedule-card" : ""} ${options.groupMember ? "schedule-group-member-card" : ""} ${options.groupFiller ? "schedule-group-filler-card" : ""} ${layoutPreview ? "layout-preview-card" : ""} ${adminArrange ? "schedule-arrange-card" : ""} ${exposeOpening ? "opening-card" : ""}" ${adminArrange && !blockChild ? `draggable="true" data-schedule-drag-id="${esc(screening.id)}" data-schedule-day="${esc(scheduleDayKey(screening))}" tabindex="0"` : (layoutPreview || adminArrange ? "" : `data-screening-card="${esc(screening.id)}"`)}>
      ${adminArrange && !blockChild ? `<div class="schedule-edit-drag-handle" aria-label="${esc(cleanMovieTitle(screening.title))} 카드 이동">⋮⋮ <span>카드 이동</span></div>` : ""}
      <div class="screening-top">
        ...
        <div class="badges compact-badges">
          ...
          ${exposeOpening ? `<span class="badge ${phase.className}">${esc(phase.label)}</span>` : `<span class="badge blue">${full ? "신청마감" : esc(screening.status || "신청 가능")}</span>`}
...
function screeningSeriesCard(series, options = {}) {
  ...
  return `
    <article class="screening-card compact-screening-card classic-schedule-card continuous-series-card screening-category-color-${categoryColor} ${adminArrange ? "schedule-arrange-card" : ""}" ${adminArrange && !blockChild ? `draggable="true" data-schedule-drag-id="${esc(screening.id)}" data-schedule-day="${esc(scheduleDayKey(screening))}" tabindex="0"` : ""}>
      ${adminArrange && !blockChild ? `<div class="schedule-edit-drag-handle" aria-label="${esc(cleanMovieTitle(screening.title))} 연속상영 카드 이동">⋮⋮ <span>묶음 이동</span></div>` : ""}
      <div class="screening-top">
...
function screeningListRow(screening) {
  ...
  return `
    <article class="all-screening-row ${full ? "is-full" : ""}" data-screening-card="${esc(screening.id)}" tabindex="0" role="button" aria-label="${esc(cleanMovieTitle(screening.title))} 영화 소개와 관람 신청 열기">
      ...
      <div class="all-screening-time">${esc(formatTimePart(screening.startTime))}</div>
      <div class="all-screening-title"><strong>${esc(cleanMovieTitle(screening.title))}</strong><span>${esc(progress)}</span></div>
      <div class="all-screening-venue">${esc(screening.venue)}</div>
      <div class="all-screening-status"><span class="badge ${info.className}">${full ? "신청마감" : esc(String(info.text).startsWith("여유 ") ? `신청가능 · ${info.text}` : info.text)}</span></div>
      <button class="btn btn-dark all-screening-book" type="button" ${full ? "disabled" : `data-action="book" data-id="${esc(screening.id)}"`}>${full ? "신청마감" : "관람신청"}</button>
    </article>`;
}
...
function renderStaff(preselectedId = "") {
  ...
  return `
    ...
    `).join("")}
    <section class="card">
      <div class="section-title"><div><h2>영화별 통계</h2><p>영화별 신청, 참석, 미참석 현황입니다.</p></div></div>
      ${statsTable(movieStats, ["영화", "회차", "정원", "신청", "참석", "미참석", "신청률", "참석률"])}
    </section>
  `;
}
...
function submitStaffLogin(form) {
  ...
  if (data.screeningId === "__all__") {
    if (String(data.pin || "") !== String(state.masterStaffPin || "")) return toast("마스타스탭 비밀번호가 맞지 않습니다.");
...
function submitStaffPinChange(form) {
  ...
  if (session.isMaster === true) {
    if (String(data.currentPin || "") !== String(state.masterStaffPin || "")) return toast("현재 비밀번호가 맞지 않습니다.");
...
function adminOverview() {
  ...
  return `
    <section class="metric-grid overview-metrics fixed-row-metrics">
      ...
      <div class="metric-card"><div class="metric-label">미참석 인원</div><div class="metric-value">${totals.totalUnattendedSeats}</div><div class="metric-note">미참석 처리</div></div>
    ...
    <section class="card">
      <div class="section-title"><div><h2>영화별 통계</h2><p>영화를 기준으로 신청, 참석, 미참석을 집계합니다. 후원 신청 ${donationStats().count}건 · 총 ${donationStats().amountText}.</p></div></div>
      ${statsTable(groupByMovie(), ["영화", "회차", "정원", "신청", "참석", "미참석", "신청률", "참석률"])}
    </section>
            ...
            <tr class="master-staff-row">
              ...
              <td>
                <div class="staff-pin-admin-row">
                  <input class="input" type="text" value="${esc(state.masterStaffPin || "")}" data-master-staff-pin-input placeholder="마스타스탭 비밀번호" />
  ...
  return `
    <section class="card opening-admin-card">
      <div class="section-title">
        ...
        <a class="btn btn-outline" href="/?v=141&fresh=1#/opening">관객 화면 보기</a>
      </div>
      <form id="openingForm">
        <div class="form-grid">
          <div>
...
function renderScheduleOrderEditor() {
  ...
  dayGroups.forEach((group) => {
    group.layout = scheduleLayoutBlocks(group.screenings, { extraRows: 2 });
  });
  return `
    <section class="card schedule-order-editor" aria-labelledby="scheduleOrderEditorTitle">
      <div class="section-title schedule-order-editor-heading">
        <div>
          ...
          <p>관객 화면에 보이는 실제 배치입니다. 카드나 그룹의 <b>이동 손잡이</b>를 빈칸으로 끌어 놓으세요. 그룹은 전체가 함께 움직이며 <b>가로·세로</b> 방향도 선택할 수 있습니다.</p>
        </div>
      ...
      <div class="classic-schedule-day-groups schedule-arrange-days">
        ${dayGroups.map((group, dayIndex) => `
          <section class="classic-schedule-day-group schedule-arrange-day schedule-day-color-${(dayIndex % 5) + 1}" data-schedule-order-day="${esc(group.key)}" data-schedule-order-grid="${esc(group.key)}">
            ...
            <div class="classic-schedule-time-groups schedule-layout-grid schedule-arrange-grid" style="--schedule-layout-rows:${group.layout.rowCount}">
              ${group.layout.blocks.map((block) => scheduleLayoutBlockHtml(block, { adminArrange: true, dayKey: group.key })).join("")}
              ${Array.from({ length: group.layout.rowCount }, (_, rowIndex) => Array.from({ length: 3 }, (_, columnIndex) => {
                const row = rowIndex + 1;
                const column = columnIndex + 1;
                if (group.layout.occupied.has(`${row}:${column}`)) return "";
                return `<button class="schedule-empty-slot schedule-grid-empty-slot" style="grid-row:${row};grid-column:${column}" type="button" data-schedule-drop-row="${row}" data-schedule-drop-column="${column}" data-schedule-day="${esc(group.key)}" aria-label="${row}행 ${column}열 빈칸: 여기에 블록을 놓으세요"><strong>＋ 빈칸</strong><span>여기에 블록 놓기</span></button>`;
              }).join("")).join("")}
            </div>
          </section>`).join("")}
      </div>
...
function participantStatsSection() {
  ...
  return `
    <section class="card participant-stats-card">
      <div class="section-title">
        <div><h2>신청자별 영화제 참여 현황</h2><p>동일한 이름 또는 전화번호를 한 사람으로 묶어 신청·참석 횟수를 집계합니다.</p></div>
      </div>
...
function reservationSmsHistoryRows() {
  const rows = [];
  state.reservations.forEach((reservation) => {
    const screening = state.screenings.find((item) => item.id === reservation.screeningId);
    const savedHistory = Array.isArray(reservation.smsHistory) ? reservation.smsHistory : [];
    if (savedHistory.length) {
      savedHistory.forEach((entry) => rows.push({ reservation, screening, ...entry }));
      return;
    }
    if (reservation.smsSentAt || reservation.smsStatus === "발송실패") {
      rows.push({
        reservation,
        screening,
        id: `legacy-reservation-${reservation.id}`,
        type: "reservation",
        status: reservation.smsStatus || "발송완료",
        sentAt: reservation.smsSentAt || reservation.updatedAt || reservation.createdAt || "",
        message: reservationConfirmationMessage(reservation, screening, reservation.status),
        requestId: reservation.smsRequestId || "",
        error: reservation.smsError || ""
      });
    }
    if (reservation.noticeSmsSentAt || reservation.noticeSmsStatus === "발송실패") {
      rows.push({
        reservation,
        screening,
        id: `legacy-notice-${reservation.id}`,
        type: "notice",
        status: reservation.noticeSmsStatus || "발송완료",
        sentAt: reservation.noticeSmsSentAt || reservation.updatedAt || reservation.createdAt || "",
        message: "별도 안내문자",
        requestId: "",
        error: reservation.noticeSmsError || ""
      });
    }
  });
...
function reservationSmsHistorySection() {
  const rows = reservationSmsHistoryRows();
  return `
    <section class="sms-history-panel" aria-label="문자전송내역">
      <div class="sms-history-heading">
        <div><h3>문자전송내역</h3><p>최근 전송부터 표시합니다. 각 신청자에게 바로 다시 보낼 수 있습니다.</p></div>
        <strong>${rows.length}건</strong>
      </div>
      ${rows.length ? `
        <div class="table-wrap sms-history-table-wrap">
          <table class="sms-history-table">
            <thead><tr><th>전송일시</th><th>구분</th><th>신청자</th><th>영화 / 상영관</th><th>결과</th><th>내용</th><th>바로 보내기</th></tr></thead>
            <tbody>${rows.map((entry) => `
              <tr>
                <td>${esc(formatDateTime(entry.sentAt))}</td>
                <td><span class="badge ${entry.type === "notice" ? "blue" : "neutral"}">${entry.type === "notice" ? "별도안내" : "예약관련"}</span></td>
                <td><strong>${esc(entry.reservation.name)}</strong><br><span class="help">${esc(entry.reservation.phone || "-")}</span></td>
                <td><strong>${esc(cleanMovieTitle(entry.screening?.title || "삭제된 회차"))}</strong><br><span class="help">${esc(entry.screening?.venue || "-")}</span></td>
                <td><span class="sms-history-status ${String(entry.status).includes("완료") ? "is-success" : "is-failed"}">${esc(entry.status || "-")}</span>${entry.error ? `<br><span class="help">${esc(entry.error)}</span>` : ""}</td>
                <td class="sms-history-message">${esc(entry.message || "-")}</td>
                <td><div class="sms-history-actions"><button class="btn btn-dark btn-small" type="button" data-action="send-sms" data-id="${esc(entry.reservation.id)}">예약문자</button><button class="btn btn-primary btn-small" type="button" data-action="open-single-notice-sms" data-id="${esc(entry.reservation.id)}">별도문자</button></div></td>
              </tr>
            `).join("")}</tbody>
          </table>
        </div>
      ` : `<div class="empty">아직 문자전송내역이 없습니다.</div>`}
    </section>
  `;
}
...
function adminReservations() {
  ...
  return `
    ...
    <section class="card">
      <div class="section-title reservation-section-title">
        <div>
          <div class="reservation-title-line">
            <h2>신청자 명단</h2>
            <button class="btn btn-primary reservation-title-action" type="button" data-action="toggle-bulk-sms">${reservationSmsSelectMode ? "문자전송 닫기" : "문자전송"}</button>
            <button class="btn btn-outline reservation-title-action" type="button" data-action="toggle-sms-history">${reservationSmsHistoryOpen ? "전송내역 닫기" : "문자전송내역확인"}</button>
          </div>
      ...
      ${reservationSmsSelectMode ? `
        <section class="bulk-sms-panel" aria-label="예약 문자 일괄 발송">
          <div class="bulk-sms-guide">
            <strong>상영관 앞 체크박스로 현재 명단을 전체 선택·해제할 수 있습니다.</strong>
            <p>개별 신청자를 선택한 뒤 예약 관련 문자를 다시 보내거나 별도 안내문자를 작성해 발송하세요.</p>
            <span class="bulk-sms-selected-count" id="bulkSmsSelectedCount">선택 0명</span>
          </div>
          <div class="bulk-sms-actions bulk-sms-send-row">
            <button class="btn btn-dark bulk-sms-send-button" type="button" data-action="bulk-reservation-sms">예약관련문자 보내기</button>
            <button class="btn btn-primary bulk-sms-send-button" type="button" data-action="open-bulk-notice-sms">별도안내문자 보내기</button>
          </div>
        </section>
      ` : ""}
      ${reservationSmsHistoryOpen ? reservationSmsHistorySection() : ""}
  ...
  return `
    ...
    <div class="table-wrap reservation-table-wrap">
      <table class="reservation-table ${options.smsSelectMode ? "reservation-table-sms" : ""}">
        <thead>
          <tr>
            ${options.smsSelectMode ? `<th class="screen-only sms-check-col"><input type="checkbox" class="reservation-check-all" data-action="toggle-visible-reservation-checks" ${allVisibleSelected ? "checked" : ""} aria-label="현재 명단 전체 선택 또는 해제" title="현재 명단 전체 선택 또는 해제" /></th>` : ""}<th class="col-venue">상영관</th><th class="col-screening">영화 / 시간</th><th class="col-applicant">신청자</th><th class="col-reservation-no">예약번호</th><th class="col-seats">인원</th><th class="col-created">신청일</th><th class="col-note">메모</th><th class="screen-only col-manage">관리</th>
          </tr>
        </thead>
        <tbody>
  ...
  return `
    <section class="card donation-stats-card">
      ...
      <section class="metric-grid compact-metric-grid donation-metrics fixed-row-metrics">
        ...
        <div class="metric-card"><div class="metric-label">확인 필요</div><div class="metric-value">${stats.pending + stats.failed}</div><div class="metric-note">미발송/실패</div></div>
      </section>
...
function groupByMovie() {
  ...
  state.screenings.forEach((screening) => {
    ...
    const current = map.get(key) || { name: key, screenings: 0, capacity: 0, confirmed: 0, attended: 0, unattended: 0 };
    ...
    current.unattended += movieReservations.filter(isUnattendedReservation).reduce((sum, r) => sum + Number(r.seats || 0), 0);
    map.set(key, current);
  });
...
function groupByVenue() {
  ...
  state.screenings.forEach((screening) => {
    ...
    const current = map.get(key) || { name: key, screenings: 0, capacity: 0, applications: 0, applicants: 0, confirmed: 0, attended: 0, canceledApplications: 0, canceledSeats: 0, unattended: 0 };
    current.screenings += 1;
    current.capacity += Number(screening.capacity || 0);
    ...
    current.unattended += canceledSeats(screening.id);
    map.set(key, current);
  });
  return [...map.values()].map((item) => ({
    ...item,
...
function statsTable(rows, headers) {
  ...
  const totals = rows.reduce((acc, row) => {
    ...
    acc.unattended += Number(row.unattended || 0);
    return acc;
  }, { screenings: 0, capacity: 0, confirmed: 0, attended: 0, unattended: 0 });
  ...
  return `
    <div class="table-wrap stats-table-wrap">
      <table class="stats-table">
        ...
        <tfoot><tr class="total-row"><td><strong>누적 합계</strong></td><td>${totals.screenings}</td><td>${totals.capacity}</td><td>${totals.confirmed}</td><td>${totals.attended}</td><td>${totals.unattended}</td><td>${totalRate}%</td><td>${totalAttendanceRate}%</td></tr></tfoot>
      </table>
    </div>
  `;
}
    ...
    <section class="card admin-backup-fixed-panel" id="adminBackupFixedPanel">
      ...
      <form id="driveSyncQuickForm" class="drive-sync-form admin-backup-fixed-form">
        ...
        <div class="form-actions admin-backup-fixed-actions">
          ...
          <a class="btn btn-dark" href="/backup.html?v=141">별도 백업페이지 열기</a>
        </div>
      </form>
    </section>
  `;
...
function clearSurveyResponses() {
  ...
  state.surveyDispatches = (state.surveyDispatches || []).map((dispatch) => {
    ...
    if (String(next.status || "").includes("응답")) next.status = next.sentAt ? "발송완료" : (next.test ? "테스트생성" : "미발송");
    return next;
  });
  persist({ autoSync: false });
...
function movieBookingDescriptionHtml(notes) {
  const text = String(notes || "").replaceAll("\r\n", "\n").trim();
  if (!text) return "";
  const lines = text.split("\n").map((line) => line.trim()).filter(Boolean);
  const eventLineIndex = lines.findIndex((line) => /^이벤트(?:\s*[:：·|\-]\s*|\s+|$)/.test(line));
  if (eventLineIndex >= 0) {
    const eventLine = lines.splice(eventLineIndex, 1)[0];
    const eventDescription = eventLine.replace(/^이벤트(?:\s*[:：·|\-]\s*|\s+)?/, "").trim();
    const description = lines.join("\n");
    return `<div class="movie-booking-description has-event" aria-label="영화 소개와 기타 안내">
      <strong>영화 소개 · 기타 안내</strong>
      <div class="movie-booking-event-row"><span class="movie-booking-event-label">이벤트</span>${eventDescription ? `<span class="movie-booking-event-text">${esc(eventDescription)}</span>` : ""}</div>
      ${description ? `<p>${formatMessageHtml(description)}</p>` : ""}
    </div>`;
  }
  return `<div class="movie-booking-description" aria-label="영화 소개와 기타 안내"><strong>영화 소개 · 기타 안내</strong><p>${formatMessageHtml(text)}</p></div>`;
}
  ...
  return `
    <section class="movie-booking-intro ${full ? "is-full" : ""}" aria-label="영화 소개">
      <div class="badges">
        ...
        <span class="badge ${statusInfo(screening).className}">${full ? "신청마감" : esc(statusInfo(screening).text)}</span>
      </div>
      <h3>${esc(cleanMovieTitle(screening.title))}</h3>
      <p class="movie-booking-time"><strong>${esc(formatDateTime(screening.startTime))}</strong></p>
      ${screening.bookingGroup ? `<div class="forced-booking-notice"><strong>${esc(screening.bookingGroup)}</strong><span>이 연속상영 그룹에서는 한 회차만 예약할 수 있습니다.</span></div>` : ""}
      ${movieBookingDescriptionHtml(screening.notes)}
      <dl class="movie-booking-facts">
        ...
        ${screening.moderator ? `<div><dt>모더레이터</dt><dd>${esc(screening.moderator)}</dd></div>` : ""}
        <div><dt>정원</dt><dd>${Number(screening.capacity || 0)}명</dd></div>
        <div><dt>신청인원</dt><dd>${confirmedSeats(screening.id)}명</dd></div>
      </dl>
      ${full ? `<div class="closed-booking-notice"><strong>현재 이 상영관은 신청이 마감되었습니다.</strong></div>` : ""}
    </section>`;
}
...
function openOpeningBooking(screening) {
  ...
  if (remainingSeats(screening) <= 0) return toast("현재 이 상영관은 신청이 마감되었습니다.");
  ...
  body.innerHTML = `
    ...
    <div class="card compact" style="margin-bottom:16px;">
      ...
      <p class="help">정원 ${Number(screening.capacity || 0)}명 · 신청 ${confirmedSeats(screening.id)}명 · 잔여 인원 ${Math.max(remainingSeats(screening), 0)}명</p>
    </div>
    ...
    <form id="bookingForm" data-id="${esc(screening.id)}" data-opening="true" data-ticket-type="${esc(phase.ticketType)}">
      <div class="form-grid">
        ...
        <div>
          ...
          <input class="input" id="guestPhone" name="phone" inputmode="tel" autocomplete="tel" required placeholder="휴대폰 번호 입력" />
        </div>
        <div>
  ...
  body.innerHTML = `
    ...
    <div class="continuous-series-time-options" aria-label="연속상영 회차 선택">
      ${series.screenings.map((slot) => {
        ...
        const unavailable = full || slot.status === "준비중" || slot.status === "마감";
        return `<button class="continuous-series-time-option ${full ? "is-full" : ""}" type="button" data-action="book" data-id="${esc(slot.id)}" ${unavailable ? "disabled" : ""}>
          ...
          <span>${full ? "신청마감" : esc(info.text)}</span>
...
function openBooking(screeningId) {
  ...
  if (remaining <= 0) return toast("현재 이 상영관은 신청이 마감되었습니다.");
  const modal = document.getElementById("bookingModal");
  const body = document.getElementById("bookingBody");
  ...
  body.innerHTML = `
    ...
    <div class="card compact" style="margin-bottom:16px;">
      ...
      <p><strong>${esc(formatDateTime(screening.startTime))}</strong></p>
      <p class="help">정원 ${Number(screening.capacity || 0)}명 · 신청 ${confirmedSeats(screening.id)}명 · 잔여 인원 ${Math.max(remaining, 0)}명</p>
    </div>
...
function openRoster(screeningId) {
  ...
  body.innerHTML = `
    <div class="card compact" style="margin-bottom:16px;">
      ...
      <p class="help">정원 ${Number(screening.capacity || 0)}명 · 신청 ${applicationCount(screening.id)}건/${appliedSeats(screening.id)}명 · 실제 참석 ${attendedApplicationCount(screening.id)}건/${actualAttendees(screening.id)}명</p>
    </div>
...
function updateScreeningList() {
  ...
  const filtered = sortedScreenings().filter((screening) => {
    const text = `${screening.category} ${screening.bookingGroup || ""} ${screening.title} ${screening.venue} ${screening.director} ${screening.ageRating} ${screening.gvHost} ${screening.moderator} ${screening.staff}`.toLowerCase();
    const info = statusInfo(screening);
  ...
  list.innerHTML = filtered.length ? `
    <div class="all-screening-list-head" aria-hidden="true">
      <span>날짜·요일</span><span>시간</span><span>영화제목·모더레이터</span><span>상영관</span><span>신청상태</span><span>신청</span>
    </div>
    ${filtered.map(screeningListRow).join("")}` : `<div class="empty">조건에 맞는 상영작이 없습니다.</div>`;
}
...
function updateBulkSmsSelectedCount() {
  ...
  const headerCheckbox = document.querySelector(".reservation-check-all");
  if (headerCheckbox) {
    const visibleIds = visibleReservationIdsForSms();
    const selectedVisibleCount = visibleIds.filter((id) => selectedReservationSmsIds.has(id)).length;
    headerCheckbox.checked = Boolean(visibleIds.length) && selectedVisibleCount === visibleIds.length;
    headerCheckbox.indeterminate = selectedVisibleCount > 0 && selectedVisibleCount < visibleIds.length;
  }
}
...
function toggleReservationSmsHistory() {
  reservationSmsHistoryOpen = !reservationSmsHistoryOpen;
  render();
}
...
async function sendNoticeSms(reservation, message) {
  ...
  if (!phone || phone.length < 10) {
    ...
    reservation.noticeSmsError = "연락처 확인 필요";
    appendReservationSmsHistory(reservation, { type: "notice", status: "발송실패", message, error: reservation.noticeSmsError });
    return false;
  }
  ...
  try {
    ...
    reservation.noticeSmsSentAt = sentAt;
    reservation.noticeSmsError = "";
    appendReservationSmsHistory(reservation, { type: "notice", status: "발송완료", sentAt, message, requestId: result.requestId || "" });
    return true;
  } catch (error) {
    reservation.noticeSmsStatus = "발송실패";
...
function smsStatusLabel(reservation) {
  ...
  if (status.includes(LEGACY_WAIT_STATUS)) return "문자 미발송";
  return "문자 미발송";
}
...
function smsStatusClass(reservation) {
  ...
  if (status.includes(LEGACY_WAIT_STATUS)) return "neutral";
  return "neutral";
}
...
function reservationConfirmationMessage(reservation, screening, status) {
  ...
  const lines = [
    `${reservation.name} 님 예약이 완료되었습니다.`,
...
function smsStatusText(reservation) {
  ...
  if (String(reservation?.smsStatus || "").includes(LEGACY_WAIT_STATUS)) return "미발송";
  if (reservation?.smsStatus === "발송실패") return `발송실패${reservation.smsError ? ` · ${reservation.smsError}` : ""}`;
  return reservation?.smsStatus || "미발송";
}

function appendReservationSmsHistory(reservation, entry = {}) {
  if (!reservation) return;
  if (!Array.isArray(reservation.smsHistory)) reservation.smsHistory = [];
  reservation.smsHistory.push({
    id: uid("sms"),
    type: entry.type === "notice" ? "notice" : "reservation",
  ...
  if (reservation.smsHistory.length > 100) reservation.smsHistory = reservation.smsHistory.slice(-100);
}
...
async function sendReservationSms(reservationOrId, screening = null, options = {}) {
  ...
  const message = reservationConfirmationMessage(reservation, targetScreening, reservation.status);
  if (!payload.phone || payload.phone.length < 10) {
    appendReservationSmsHistory(reservation, { type: "reservation", status: "발송실패", message, error: "연락처 확인 필요" });
  ...
  if (protocol !== "http:" && protocol !== "https:") {
    appendReservationSmsHistory(reservation, { type: "reservation", status: "미발송", message, error: "로컬 파일에서는 SENS API 호출 불가" });
    updateReservationSmsState(reservation, { smsStatus: "미발송", smsError: "로컬 파일에서는 SENS API 호출 불가" });
  ...
  try {
    ...
    const sentAt = new Date().toISOString();
    appendReservationSmsHistory(reservation, { type: "reservation", status: "발송완료", sentAt, message, requestId: result.requestId || "" });
    ...
    const errorMessage = String(error?.message || error).slice(0, 80);
    appendReservationSmsHistory(reservation, { type: "reservation", status: "발송실패", message, error: errorMessage });
    updateReservationSmsState(reservation, {
      smsStatus: "발송실패",
...
function showReservationComplete(reservation, status) {
  ...
  const title = openingReservation ? "개막작 신청이 확정되었습니다." : "신청이 확정되었습니다.";
...
function submitOpeningBooking(form, screening, data) {
  ...
  if (status !== "확정") return toast("현재 이 상영관은 신청이 마감되었습니다.");
  ...
  closeModals();
  render();
  const message = `개막작 신청이 확정되었습니다. 예약번호: ${reservationDisplayNumber(reservation, screening)}`;
  toast(message);
...
function submitBooking(form) {
  ...
  closeModals();
  render();
  const message = `신청이 확정되었습니다. 예약번호: ${reservationDisplayNumber(reservation, screening)}`;
  toast(message);
  showReservationComplete(reservation, status);
  autoSendReservationSms(reservation, screening, status);
}
...
async function submitAdminLogin(form) {
  ...
  if (master) {
    const localPreview = ["localhost", "127.0.0.1"].includes(window.location.hostname)
      && new URLSearchParams(window.location.search).get("masterPreview") === "1";
    let authorized = localPreview && pin.length > 0;
    try {
      if (!authorized) {
        const response = await fetch(ADMIN_AUTH_API_ENDPOINT, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ role: "master", pin })
        });
        const result = await response.json().catch(() => ({}));
        authorized = response.ok && result.ok === true;
      }
    } catch (error) {
      toast("로그인 확인 중 오류가 발생했습니다.");
      return;
    }
    if (authorized) {
...
function scheduleLayoutForDay(dayKey, options = {}) {
  return scheduleLayoutBlocks(
    state.screenings.filter((screening) => scheduleDayKey(screening) === dayKey),
    options
  );
}

function scheduleBlockCells(block, row = block.row, column = block.column, orientation = block.orientation) {
  const dimensions = scheduleLayoutBlockDimensions({ ...block, orientation });
  const cells = [];
  for (let r = row; r < row + dimensions.rows; r += 1) {
    for (let c = column; c < column + dimensions.columns; c += 1) cells.push(`${r}:${c}`);
  }
  return { ...dimensions, cells };
}

function scheduleBlockCanOccupy(layout, block, row, column, orientation = block.orientation, ignoredBlockIds = []) {
  const ignored = new Set(ignoredBlockIds);
  const placement = scheduleBlockCells(block, row, column, orientation);
  if (row < 1 || column < 1 || column + placement.columns - 1 > layout.columns) return false;
  const occupied = new Set();
  layout.blocks.forEach((other) => {
    if (ignored.has(other.id)) return;
    scheduleBlockCells(other).cells.forEach((cell) => occupied.add(cell));
  });
  return placement.cells.every((cell) => !occupied.has(cell));
}

function firstOpenScheduleBlockPosition(layout, block, orientation = block.orientation, ignoredBlockIds = []) {
  for (let row = 1; row <= Math.max(layout.rowCount + 12, 20); row += 1) {
    for (let column = 1; column <= layout.columns; column += 1) {
      if (scheduleBlockCanOccupy(layout, block, row, column, orientation, ignoredBlockIds)) return { row, column };
    }
  }
  return null;
}

function saveScheduleBlockLayout(block, values = {}) {
  const row = screeningScheduleGridNumber(values.row);
  const column = screeningScheduleGridNumber(values.column, 3);
  const orientation = values.orientation === "vertical" ? "vertical" : "horizontal";
  block.screenings.forEach((screening) => {
    screening.scheduleRow = row;
    screening.scheduleColumn = column;
    screening.scheduleGroupOrientation = orientation;
  });
}

function moveScheduleBlock(dayKey, blockId, row, column, targetBlockId = "") {
  if (!isMasterAdminAuthed()) return toast("최고관리자만 시간표 배치를 바꿀 수 있습니다.");
  const layout = scheduleLayoutForDay(dayKey, { extraRows: 2 });
  const source = layout.blocks.find((block) => block.id === blockId);
  if (!source) return toast("옮길 카드 그룹을 찾지 못했습니다.");
  const target = targetBlockId ? layout.blocks.find((block) => block.id === targetBlockId) : null;
  const destinationRow = screeningScheduleGridNumber(row);
  const destinationColumn = screeningScheduleGridNumber(column, 3);
  if (!destinationRow || !destinationColumn) return;

  if (target && target.id !== source.id) {
    const sourceFits = scheduleBlockCanOccupy(layout, source, target.row, target.column, source.orientation, [source.id, target.id]);
    const targetFits = scheduleBlockCanOccupy(layout, target, source.row, source.column, target.orientation, [source.id, target.id]);
    if (!sourceFits || !targetFits) return toast("두 블록의 크기가 달라 바로 맞바꿀 수 없습니다. 점선 빈칸으로 옮겨 주세요.");
    saveScheduleBlockLayout(source, { row: target.row, column: target.column, orientation: source.orientation });
    saveScheduleBlockLayout(target, { row: source.row, column: source.column, orientation: target.orientation });
  } else {
    if (!scheduleBlockCanOccupy(layout, source, destinationRow, destinationColumn, source.orientation, [source.id])) {
      return toast("그 위치에는 블록 전체가 들어갈 빈공간이 부족합니다.");
    }
    saveScheduleBlockLayout(source, { row: destinationRow, column: destinationColumn, orientation: source.orientation });
  }
  persist();
  render();
  toast(source.screenings.length > 1 ? "그룹 전체 위치를 저장했습니다." : "카드 위치를 저장했습니다.");
}

function setScheduleBlockOrientation(dayKey, blockId, orientation) {
  if (!isMasterAdminAuthed()) return toast("최고관리자만 시간표 배치를 바꿀 수 있습니다.");
  const nextOrientation = orientation === "vertical" ? "vertical" : "horizontal";
  const layout = scheduleLayoutForDay(dayKey, { extraRows: 2 });
  const block = layout.blocks.find((item) => item.id === blockId);
  if (!block || block.orientation === nextOrientation) return;
  let position = { row: block.row, column: block.column };
  if (!scheduleBlockCanOccupy(layout, block, position.row, position.column, nextOrientation, [block.id])) {
    position = firstOpenScheduleBlockPosition(layout, block, nextOrientation, [block.id]);
  }
  if (!position) return toast("방향을 바꿀 수 있는 빈공간을 찾지 못했습니다.");
  saveScheduleBlockLayout(block, { ...position, orientation: nextOrientation });
  persist();
  render();
  toast(`그룹을 ${nextOrientation === "vertical" ? "세로" : "가로"} 방향으로 배치했습니다.`);
}
...
function resetScheduleOrder() {
  ...
  state.screenings.forEach((screening) => {
    screening.scheduleOrder = null;
    screening.scheduleRow = null;
    screening.scheduleColumn = null;
    screening.scheduleGroupOrientation = "horizontal";
  });
  persist();
  render();
...
function buildStatsRows() {
  ...
  rows.push([]);
  rows.push(["영화별 통계"]);
  rows.push(["영화", "회차", "정원", "신청", "참석", "미참석", "신청률", "참석률"]);
  groupByMovie().forEach((row) => rows.push([row.name, row.screenings, row.capacity, row.confirmed, row.attended, row.unattended, `${row.rate}%`, `${row.attendanceRate}%`]));
  rows.push([]);
  rows.push(["날짜별 통계"]);
  rows.push(["날짜", "회차", "정원", "신청", "참석", "미참석", "신청률", "참석률"]);
  groupByDate().forEach((row) => rows.push([row.name, row.screenings, row.capacity, row.confirmed, row.attended, row.unattended, `${row.rate}%`, `${row.attendanceRate}%`]));
  rows.push([]);
  rows.push(["후원자 집계"]);
...
function buildDriveStats() {
  return sortedScreenings().map((s) => {
    ...
    return {
      ...
      scheduleRow: s.scheduleRow || "",
      scheduleColumn: s.scheduleColumn || "",
      scheduleGroupOrientation: s.scheduleGroupOrientation || "horizontal",
...
document.addEventListener("dragstart", (event) => {
  const block = event.target.closest?.(".schedule-layout-edit-block[data-schedule-block-id]");
  if (block && isMasterAdminAuthed()) {
    if (event.target.closest?.("button")) {
      event.preventDefault();
      return;
    }
    draggedScheduleBlockId = block.dataset.scheduleBlockId || "";
    draggedScheduleCardId = "";
    block.classList.add("is-dragging");
    if (event.dataTransfer) {
      event.dataTransfer.effectAllowed = "move";
      event.dataTransfer.setData("text/plain", draggedScheduleBlockId);
    }
    return;
  }
...
document.addEventListener("dragover", (event) => {
  if (draggedScheduleBlockId) {
    const grid = event.target.closest?.("[data-schedule-order-grid]");
    if (!grid) return;
    const sourceBlock = document.querySelector(`.schedule-layout-edit-block[data-schedule-block-id="${CSS.escape(draggedScheduleBlockId)}"]`);
    if (!sourceBlock || grid.dataset.scheduleOrderGrid !== sourceBlock.dataset.scheduleDay) return;
    event.preventDefault();
    if (event.dataTransfer) event.dataTransfer.dropEffect = "move";
    document.querySelectorAll(".schedule-layout-edit-block.is-drop-target, .schedule-empty-slot.is-drop-target").forEach((item) => item.classList.remove("is-drop-target"));
    const dropTarget = event.target.closest?.(".schedule-layout-edit-block[data-schedule-block-id], [data-schedule-drop-row]");
    if (dropTarget?.dataset.scheduleBlockId !== draggedScheduleBlockId) dropTarget?.classList.add("is-drop-target");
...
document.addEventListener("drop", (event) => {
  if (draggedScheduleBlockId) {
    ...
    if (!grid) return;
    event.preventDefault();
    const sourceId = draggedScheduleBlockId;
    const sourceBlock = document.querySelector(`.schedule-layout-edit-block[data-schedule-block-id="${CSS.escape(sourceId)}"]`);
    const dayKey = sourceBlock?.dataset.scheduleDay || "";
    const emptySlot = event.target.closest?.("[data-schedule-drop-row]");
    const targetBlock = event.target.closest?.(".schedule-layout-edit-block[data-schedule-block-id]");
    const row = Number(emptySlot?.dataset.scheduleDropRow || targetBlock?.dataset.scheduleBlockRow || 0);
    const column = Number(emptySlot?.dataset.scheduleDropColumn || targetBlock?.dataset.scheduleBlockColumn || 0);
    const targetId = targetBlock?.dataset.scheduleBlockId || "";
    draggedScheduleBlockId = "";
    if (dayKey && row && column && sourceId !== targetId) moveScheduleBlock(dayKey, sourceId, row, column, targetId);
    return;
  }
  if (!draggedScheduleCardId) return;
  const grid = event.target.closest?.("[data-schedule-order-grid]");
...
document.addEventListener("dragend", () => {
  draggedScheduleCardId = "";
  draggedScheduleBlockId = "";
  document.querySelectorAll(".schedule-arrange-card.is-dragging, .schedule-arrange-card.is-drop-target, .schedule-layout-edit-block.is-dragging, .schedule-layout-edit-block.is-drop-target, .schedule-empty-slot.is-drop-target").forEach((item) => item.classList.remove("is-dragging", "is-drop-target"));
...
document.addEventListener("click", (event) => {
  ...
  if (action === "go-home") {
    event.preventDefault();
    if (isAdminRoutePath() || (window.location.pathname && window.location.pathname !== "/")) window.location.href = "/?v=141&fresh=1";
  ...
  if (action === "toggle-sms-history") toggleReservationSmsHistory();
  ...
  if (action === "set-schedule-block-orientation") setScheduleBlockOrientation(button.dataset.day || "", button.dataset.blockId || "", button.dataset.orientation || "horizontal");
  ...
  if (action === "reset-demo") {
    ...
    state = normalizeState({ screenings: cloneData(seedScreenings), reservations: cloneData(seedReservations), donations: [], sponsorClicks: 0, adminPin: String(state.adminPin || ""), generalAdmins: cloneData(state.generalAdmins || []), masterStaffPin: String(state.masterStaffPin || ""), masterStaffPresent: false, lastUpdated: new Date().toISOString() });
    selectedScreeningId = null;
    persist();
    render();
...
document.addEventListener("keydown", (event) => {
  const bookingRow = event.target.closest?.(".all-screening-row[data-screening-card]");
  if (bookingRow && event.target === bookingRow && ["Enter", " "].includes(event.key)) {
    event.preventDefault();
    openBooking(bookingRow.dataset.screeningCard);
    return;
  }
