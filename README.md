...
- 신청자 명단은 상영관·상영시간·신청자 이름 순으로 정렬됩니다.
- 신청자 엑셀저장용 CSV에 문자 수신 동의, 문자 상태, 발송일, SENS 요청 ID를 추가했습니다.
...
- 후원자 사전예약 기간에는 후원에 참여하는 관객이 먼저 개막작 예약을 신청할 수 있습니다.
- 신청 화면에는 `박정민배우를 가장 가까이서 만날 수 있는 후원자님들을 위한 사전예약` 문구를 반영했습니다.
- 후원자 사전예약 확정자는 자동으로 지정좌석이 배정됩니다.
- 일반석 오픈 이후 신청자는 자유석으로 접수됩니다.
- 지정좌석 수, 총 정원, 후원자 사전예약 시작/종료, 일반석 오픈 시작/종료는 관리자 화면에서 수정할 수 있습니다.

## 기존 주요 기능

- QR 첫 화면에서 `개막작 티켓팅`, `영화 신청하기`, `후원하기`로 이동합니다.
- 후원 양식 제출 시 후원 정보를 저장하고 감사 문자 발송을 시도한 뒤 `https://aq.gy/f/2hekV` 계좌이체 화면으로 이동합니다.
...
- 각 상영 회차에 담당 스태프 비밀번호를 설정할 수 있습니다.
- 관객 신청 카드의 `신청자 확인` 버튼 또는 상단 `스태프` 메뉴로 담당자 화면에 진입합니다.
- 담당자는 로그인한 자신의 상영 회차 신청자만 확인하고 확정·대기·취소 및 현장 참석 확인을 처리할 수 있습니다.
- 같은 담당자명과 같은 비밀번호가 설정된 여러 회차는 한 번 로그인하여 함께 관리합니다.
- 모든 상영 회차의 최초 스태프 비밀번호는 `0909`입니다. 담당 스태프는 로그인 후 자신의 회차 비밀번호를 직접 변경할 수 있습니다.

> 중요: 현재 버전의 예약 데이터는 브라우저 `localStorage`에 저장됩니다. 관객과 스태프가 서로 다른 휴대폰·PC를 사용하면 신청 정보가 공유되지 않습니다. 실제 영화제 운영에서 여러 담당자가 같은 신청자 명단을 보려면 Supabase 등 중앙 데이터베이스 연결과 서버 인증이 반드시 필요합니다. 현재 스태프 비밀번호 기능은 화면 흐름과 권한 범위를 확인하기 위한 프런트엔드형 구현입니다.


### v52 문자 선택 / 수정 예약문자 보강
- 신청자 명단의 문자 발송 버튼명을 `문자전송하기`로 변경했습니다.
- 문자 선택 모드에서 현재 목록 전체해제와 전체해제를 지원합니다.
- 예약상황 수정 후 예약문자를 보내면 현재 저장된 정보 기준으로 문자가 발송됩니다.


## v53 변경

- 신청자/통계 내보내기 버튼 문구를 엑셀저장으로 정리했습니다.
- 관리자 화면에서 신청자현황과 통계를 구글드라이브/구글시트로 저장할 수 있는 연동 버튼을 추가했습니다.
- 설정 방법은 `GOOGLE_DRIVE_SYNC_GUIDE.md`를 확인하세요.
