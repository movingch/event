...
function doGet(e) {
  try {
    ...
    if (e && e.parameter && e.parameter.action === 'exportData') {
      return jsonOutput({ ok: true, message: '구글시트 원본 데이터 내보내기 완료', version: 'v95-sheet-source', data: exportCurrentData_(ss) });
    }
  ...
  return jsonOutput({ ok: true, message: '머내마을영화제 구글시트 연동 웹앱 v95 원본 데이터 제공 버전이 작동 중입니다.' });
}
